...
with open("D:/(4) WhiteHatJr. Codes/Third Module/Normal Distribution/Project/Student Performance/Students_Performance.csv", newline="") as f:
    ...
    performance_list_reading = []

    for i in range(len(reader)):
        read = reader[i][6]
        performance_list_reading.append(float(read))

len_list = len(performance_list_reading)

total = 0

for t in performance_list_reading:
    total += t
...
mode_of_list = s.mode(performance_list_reading)

median_of_list = s.median(performance_list_reading)

devaition = s.stdev(performance_list_reading)
...
r_list_first = [r for r in performance_list_reading if (r > d_1_left and r < d_1_right)]

percent_first_sd = len(r_list_first)/len(performance_list_reading) * 100

r_list_second = [r for r in performance_list_reading if(r > d_2_left and r < d_2_right)]

percent_second_sd = len(r_list_second)/len(performance_list_reading) * 100
...
figure = pff.create_distplot([performance_list_reading], ["Results"], show_hist=False)
